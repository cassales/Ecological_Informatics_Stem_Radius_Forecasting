...
if reduced_network:
	...
	early_stop = tf.keras.callbacks.EarlyStopping(patience=early_stop_patience+20)
	history = model.fit(x=X, y=Y, batch_size=1024, epochs=200, validation_split=0.3, callbacks=[early_stop])
	model_name=f'./CNN_Ashley_HALF_{lookback}_{temporal_res}min_{learning_rate}LR_200epochs'
	with open(f'{model_name}_history_200epochs.pkl', 'wb') as file_pi:
